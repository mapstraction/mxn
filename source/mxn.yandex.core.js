...
Mapstraction: {
    ...
    addControls: function(args) {
        ...
        if (args.overview) {
            if (typeof(args.overview) != 'number') args.overview = 5;
    ...
    addImageOverlay: function(id, src, opacity, west, south, east, north, oContext) {
        ...
        var YImageOverlay = function (imgElm) {
            ...
            this.onRemoveFromMap = function () {
                if (ymap) ymap.removeChild(imgElm);
            };
    ...
    addTileLayer: function(tile_url, opacity, copyright_text, min_zoom, max_zoom, map_type) {
        ...
        dataSource.getTileUrl = function (t,s) { return this._tileUrlTemplate.replace(/\{X\}/g,t.x).replace(/\{Y\}/g,t.y).replace(/\{Z\}/g,s)}
        ...
        if (map_type) {
            ...
            var newType = new YMaps.MapType([layerID],
                ...
                { textColor: "#706f60",
                  minZoom:   min_zoom,
                  maxZoom:   max_zoom });
            for (i in map.__controls) {
                if (map.__controls[i] instanceof YMaps.TypeControl) {
                    var tp = map.__controls[i]
                    break;
                }
            }
            if (tp == undefined) {
                var tp = new YMaps.TypeControl();
                map.addControl(tp);
            }
            tp.addType(newType);
        } else {
    ...
    toProprietary: function() {
        ...
        if (this.iconUrl) {
            ...
            if (this.iconSize) {
                ...
                if (this.iconAnchor) {
                    var anchor = new YMaps.Point(this.iconAnchor[0], this.iconAnchor[1]);
                }
                else {
                    var anchor = new YMaps.Point(0, 0);
                }
                icon.offset = anchor;
            }
        ...
        if (this.labelText)  ymarker.name = this.labelText
        if (this.infoBubble) ymarker.setBalloonContent(this.infoBubble)
